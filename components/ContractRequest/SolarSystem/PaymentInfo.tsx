...
function PaymentInfo({
  ...
  function validateFields() {
    ...
    if (!requestInfo.necessidaInscricaoRural) {
      toast.error(
        "Por favor, preencha sobre a necessidade de faturamento por inscrição rural."
      );
      return false;
    }
    ...
    if (
      ...
      requestInfo.nomeGerente.trim().length < 5
    ) {
    ...
    if (
      requestInfo.origemRecurso == "FINANCIAMENTO" &&
      requestInfo.contatoGerente.trim().length < 8
    ) {
          ...
          <div className="mt-2 flex flex-col gap-2 lg:grid lg:grid-cols-3">
                ...
                handleChange={(value) =>
                  setRequestInfo({
                    ...
                    valorParcela: requestInfo.valorContrato
                      ? Number(
                          (requestInfo.valorContrato / Number(value)).toFixed(2)
                        )
                      : 0,
                  })
                }
            ...
            <div className="col-span-3 flex items-center justify-center">
              <SelectInput
                ...
                handleChange={(value) =>
                  setRequestInfo({ ...requestInfo, formaDePagamento: value })
                }
                selectedItemLabel="NÃO DEFINIDO"
                onReset={() => {
                  setRequestInfo((prev) => ({
