...
function NewCost({ closeModal, pricing, setPricing }: NewCostProps) {
  ...
  function handleAddCost() {
    ...
    if (newCostInfo.tax < 0) {
      toast.error(
        "Por favor, preencha um valor válido para a taxa de imposto."
      );
      return;
    }
    ...
    const proposedPrice = getProposedPrice(
      newCostInfo.cost,
      newCostInfo.tax,
      newCostInfo.margin
    );
          ...
          <div className="flex grow flex-col gap-y-2 overflow-y-auto overscroll-y-auto py-1 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
            <div className="w-full self-center lg:w-[50%]">
              <SelectInput
                ...
                handleChange={(value) =>
                  setNewCostInfo((prev) => ({ ...prev, type: value }))
                }
                onReset={() =>
                  setNewCostInfo((prev) => ({ ...prev, type: undefined }))
                }
                width="100%"
              />
            </div>
