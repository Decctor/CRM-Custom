...
function PeriodDropdownFilter({
  initialAfter,
  initialBefore,
  setDateParam,
}: PeriodDropdownFilterProps) {
    ...
    <div ref={ref} className="relative flex h-[40px] w-[40px] justify-end">
      ...
      {dropdownIsOpen ? (
        <div className="absolute -right-[400%] top-[45px] z-[100] flex h-[115px] w-[350px] flex-col self-center rounded-md border border-gray-200 bg-[#fff] p-2 shadow-sm">
          <h1 className="text-center text-sm font-medium text-gray-700">
            SELECIONE UM PERÍODO DE FILTRO
          </h1>
          <div className="mt-2 flex w-full items-center justify-between">
            <div className="flex flex-col items-start">
              ...
              <input
                type="date"
                value={
                  dateFilter.after ? formatDate(dateFilter.after) : undefined
                }
            ...
            <div className="flex flex-col items-end">
              ...
              <input
                type="date"
                value={
                  dateFilter.before ? formatDate(dateFilter.before) : undefined
                }
                className="text-xs text-gray-700 outline-none"
                onChange={(e) =>
                  setDateFilter((prev) => ({
          ...
          <div className="mt-2 flex w-full justify-between">
            {msg.text ? (
              <p className={`text-xs ${msg.color} italic`}>{msg.text}</p>
            ) : (
              <div></div>
            )}
            <button
