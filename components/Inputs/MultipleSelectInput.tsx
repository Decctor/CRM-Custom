...
function MultipleSelectInput<T>({
  ...
  const [selectedIds, setSelectedIds] = useState<(string | number)[] | null>(
    getValueID(selected)
  );
        ...
        {selectMenuIsOpen ? (
          ...
          >
            {selectedIds && selectedIds.length > 0 && options
              ? options.filter((item) => selectedIds.includes(item.id)).length >
                1
                ? "MÚLTIPLAS SELEÇÕES"
                : options.filter((item) => selectedIds.includes(item.id))[0]
                    .label
              : "NÃO DEFINIDO"}
          </p>
        )}
