...
const formatKits: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  const formattedKits = Kits.map((internalKit: any) => {
    ...
    if (internalKit.inversorDoisModelo != "") {
      inversoresArr.push({
        id: Inverters.find((i) => i.modelo == internalKit.inversorDoisModelo)
          ?.id,
    ...
    if (internalKit.inversorTresModelo != "") {
      inversoresArr.push({
        id: Inverters.find((i) => i.modelo == internalKit.inversorTresModelo)
          ?.id
          ? Inverters.find((i) => i.modelo == internalKit.inversorTresModelo)
              ?.id
          : "N/A",
    ...
    modulosArr.push({
      id: Modules.find((i) => i.modelo == internalKit.moduloModelo)?.id
        ? Modules.find((i) => i.modelo == internalKit.moduloModelo)?.id
        : "N/A",
    ...
    const obj = {
      ...
      categoria: "ON-GRID",
      topologia:
        internalKit.topologia == "tradicional" ? "INVERSOR" : "MICRO-INVERSOR",
      preco:
        typeof internalKit.preco == "string"
          ? Number(internalKit.preco.replace(",", "."))
          : null,
